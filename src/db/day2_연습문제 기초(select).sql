...
select name
from customer
where publisher in  -- 박지성이 구매한 출판사
	(select publisher
	from book 
	where bookid in -- 박지성이 구매한 출판사 책id
		(select bookid
		from orders
		where custid in -- 박지성 id
			(select custid
			from customer
			where name like '박지성')));
...
select name
    ...
    where custid =
        ...
        where bookid = 
			(select bookid
            from book
            where count(distinct publisher) >= 2));


-- 2-(1)
insert into book colunm(bookname, publisher, price) values('스포츠 세계', '대한미디어', 10000);
-- 2-(2)
drop 
